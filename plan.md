...
### 🎰 PvP 매칭 시스템 (WebSocket) [PVP-001]

#### Backend
- [x] WebSocket 엔드포인트 구현 (`/ws/pvp/match`)
- [x] 매칭 큐 시스템 (Redis pub/sub)
- [x] 30초 타임아웃 처리
...
- [x] 매칭 실패 시 솔로 미니게임 트리거 (난이도 상향)
- [x] PvP 미니게임 액션 동기화 (야바위, 나잡아봐라, 스페이스바 광클)

#### Frontend
- [x] WebSocket 연결 관리 (zustand)
- [x] 매칭 대기 UI (30초 카운트다운)
- [x] 배팅 호감도 선택 UI
- [x] 매칭 결과 모달 (승리/패배/캐릭터 획득)
- [x] "상대방의 애인을 뺏어왔습니다!" 알림
- [x] PvP 미니게임 룰렛 (게임 타입 선택 애니메이션)
- [x] 야바위 게임 (ShellGame.tsx)
- [x] 나잡아봐라 게임 (ChaseGame.tsx)
- [x] 스페이스바 광클 게임 (MashingGame.tsx)
- [x] PvP 게임 매니저 (PvPGameManager.tsx)
